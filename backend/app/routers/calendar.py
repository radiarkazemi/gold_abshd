...
async def list_holidays(db: Session = Depends(get_db), _admin=Depends(require_permission("calendar"))):
    return db.query(Holiday).order_by(Holiday.date.asc()).all()


@router.post("/api/admin/holidays", response_model=HolidayOut)
async def add_holiday(payload: HolidayIn, db: Session = Depends(get_db), _admin=Depends(require_permission("calendar"))):
...
async def remove_holiday(holiday_id: str, db: Session = Depends(get_db), _admin=Depends(require_permission("calendar"))):
