...
async def get_all_prices(_admin=Depends(require_permission("prices"))):
    ...
    Response is obfuscated the same way as the customer-facing price
    channels - see app/obfuscation.py.
    """
    try:
        url = _all_prices_url()
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            ...
            return {"payload": encode_payload(resp.json())}
    except httpx.HTTPStatusError as e:
        logger.warning(f"[admin-prices] goldbridge returned {e.response.status_code}")
        raise HTTPException(status_code=502, detail="goldbridge returned an error")
    except Exception as e:
        logger.warning(f"[admin-prices] failed to reach goldbridge: {e}")
        raise HTTPException(status_code=502, detail="couldn't reach goldbridge")